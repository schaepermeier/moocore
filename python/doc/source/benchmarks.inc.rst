...
For 3D, `Optuna`_ uses a :math:`O(n^2)` algorithm, while `moocore`_ uses the best-known :math:`O(n\log n)` one, so the gap between the two will get significantly larger with a larger number of points than those tested here. `moarchiving`_ uses the :math:`O(n\log n)` algorithm, but it is a pure Python implementation, so it is still significantly slower than `moocore`_'s C implementation.  `BoTorch`_ and `fast-pareto`_ are not included for more than 3 objectives because **they are tens of thousands of times slower** than `moocore`_.  `Nevergrad`_ and `Optuna`_ are not included for more than 5 objectives for the same reason.
